...
<template>
  <div class="income-container">
    <div class="income-island">
      <!-- 顶部信息区 -->
      <div class="income-header">
        <div class="income-dot"></div>
        <div class="income-category">{{ props.category }}<span class="income-date mx-2">{{ dateStr }}</span></div>
        <div class="income-amount">+{{ formatAmount(props.amount) }}</div>
      </div>

      <!-- AI 建议区域 -->
      <div class="income-ai" v-if="props.aiSuggestion">
        <div class="ai-icon">AI</div>
        <div class="ai-suggestion">{{ props.aiSuggestion }}</div>
      </div>

      <div class="income-theme op-50">
        <div class="theme-icon">
          <Logo class="scale-150" />
        </div>
        <div class="theme-text">聚风聚财 · 收入记录</div>
      </div>

      <!-- 操作按钮区 -->
      <div class="income-actions">
        <button class="action-btn confirm" @click="handleConfirm">记入收入</button>
